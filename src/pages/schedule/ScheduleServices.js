...
export const getendtime = async (
  ...
  var requestoptions = {
    ...
    body: JSON.stringify({
      ...
      duration: currentCellDuration,
      servicekey: activitykey,
      operator: currentEmployeeName,
    }),
  };
  const url = `${process.env.REACT_APP_BASE_URL}/Getoperation`;
  return await fetch(url, requestoptions) // Request fish
    ...
    .then((json) => {
      ...
      //throw new Error("invalid username/password");
      return {
        duration: json.user_response.returnenddate,
        keyvalue: json.user_response.activityname,
      };
    });
};
